...
Let's compare this to a simple linear search, and lets assume a situation where the item searched for is not in the target list. In a linear search, the search area is traversed from the beginning onwards, until either the item is found, or we run out of search area. The number of steps needed to cover the entire search area grows _linearly_ at the same pace as the size of the search area. If the search area is a million items long, each search step still covers only one search candidate from the beginning of the search area, and we would have to take a million search steps to make sure the item is not in the search area. 
